 ...
 *
 */
// don't necessarily have to be admin here, requireAdmin seems off
...
router.patch('/:username', async function(
  ...
  try {
    ...
    // remove token so we don't try to change it. Shouldn't be able to modify token here
    let fields = { ...req.body };
    delete fields._token;

    // fixes bug #3
    if ( fields.admin && !req.curr_admin){
      throw new ExpressError('Only admins can modify admin status', 401);
    }
